...
let update (msg: Msg) (state: Model) =
    ...
    | ReceiveFileSettings fileSettings ->
        { state with
    ...
    | Acceptance msg ->
        { state with
            acceptance = AcceptanceQueue.update msg state.acceptance },
        Cmd.none
    | Execution msg ->
        { state with
            execution = ExecutionQueue.update msg state.execution },
        Cmd.none
    | Results msg ->
        { state with
            results = Executed.update msg state.results },
        Cmd.none
